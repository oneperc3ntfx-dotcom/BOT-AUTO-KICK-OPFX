...
async def handle_kick_duration(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if user_id in data:
            ...
            if hours == 0:  # Unlimited
                data[user_id]["kick_at"] = None
                await query.edit_message_text(f"✅ User ID {user_id} diset ke Unlimited (tidak akan di-kick).")
            else:
                kick_time = join_time + timedelta(hours=hours)
                data[user_id]["kick_at"] = kick_time.isoformat()
                await query.edit_message_text(f"✅ Kick user ID {user_id} dalam {hours//24} hari berhasil diset.")
            save_data(data)
        else:
...
async def kick_user(context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id_str, user_data in list(data.items()):
        ...
        if not kick_at_str:  # Unlimited → skip
            continue
        try:
